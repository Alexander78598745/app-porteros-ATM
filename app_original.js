class GoalkeeperTracker {
    ...
    updateGoalkeeperMinutes() {
        ...
        // Solo actualizar minutos si el partido está en curso
        if (this.matchPhase === 'first_half' || this.matchPhase === 'second_half') {
            if (this.currentMatch.activeGoalkeeper === this.currentMatch.goalkeeperTitular) {
                // Calcular minutos desde el último cambio o inicio
                const lastChangeTime = this.currentMatch.activeGoalkeeperStartTime || this.startTime;
                const minutesSinceChange = Math.floor((currentTime - lastChangeTime - this.pausedTime) / 1000 / 60);
                this.currentMatch.titularMinutes = (this.currentMatch.titularMinutesBefore || 0) + minutesSinceChange;
            } else if (this.currentMatch.activeGoalkeeper === this.currentMatch.goalkeeperSuplente) {
                // Calcular minutos del suplente
                const lastChangeTime = this.currentMatch.activeGoalkeeperStartTime || this.startTime;
                const minutesSinceChange = Math.floor((currentTime - lastChangeTime - this.pausedTime) / 1000 / 60);
                this.currentMatch.suplenteMinutes = (this.currentMatch.suplenteMinutesBefore || 0) + minutesSinceChange;
            }
        }
